...
#pragma warning restore CA1416
                ...
                Dispatcher.UIThread.Invoke<Task>(async () =>
                {
                    ...
                    if (partialSub.Paragraphs.Count > 0)
                    {
                        var answer = await MessageBox.Show(
                            Window!,
                            Se.Language.Video.AudioToText.KeepPartialTranscription,
                            string.Format(Se.Language.Video.AudioToText.DoYouWantToKeepXLines, partialSub.Paragraphs.Count),
            ...
            Dispatcher.UIThread.Invoke<Task>(async () =>
            {
                ...
                if (_incompleteModel)
                {
                    await MessageBox.Show(Window!, Se.Language.Video.AudioToText.IncompleteModel,
                        Se.Language.Video.AudioToText.TheModelIsIncompletePleaseDownloadTheFull);
                    hasError = true;
                }
                else if (_unknownArgument && !string.IsNullOrEmpty(settings.WhisperCustomCommandLineArguments))
                {
                    await MessageBox.Show(Window!, string.Format(Se.Language.Video.AudioToText.UnknownArgumentX, settings.WhisperCustomCommandLineArguments),
                        Se.Language.Video.AudioToText.UnknownArgumentPleaseCheckTheAdvancedSettings);
                    hasError = true;
                }
                else if (_cudaOutOfMemory)
                {
                    await MessageBox.Show(Window!, Se.Language.Video.AudioToText.CUDAFailed,
                        Se.Language.Video.AudioToText.WhisperRanOutOfCUDAMemoryTryA);
                    hasError = true;
                }
...
#pragma warning restore CA1416
    ...
    private async Task ProcessOnlineSttTranscription(IOnlineSttEngine engine, string audioFileName, string? language = null, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(async () =>
            {
                await MessageBox.Show(Window!, string.Format(Se.Language.Video.AudioToText.XX, Se.Language.General.TranscriptionFailed, ex.Message), Se.Language.General.Error);
                IsTranscribeEnabled = true;
            });
        }
        finally
...
#pragma warning restore CA1416
    ...
    private async Task MakeResult(Subtitle? transcribedSubtitle)
    {
        ...
        else
        {
            ...
            {
                await MessageBox.Show(Window!, Se.Language.Video.AudioToText.NoResultFile,
                    Se.Language.Video.AudioToText.NoResultFileWasGeneratedByWhisperBut);
            ...
            {
                await MessageBox.Show(Window!, Se.Language.Video.AudioToText.NoTranscriptionResult,
                    Se.Language.Video.AudioToText.CrispASRFinishedWithoutGeneratingSubtitlesPleaseCheck);
...
#pragma warning restore CA1416
            ...
            if (string.IsNullOrEmpty(_videoFileName))
            {
                ...
                {
                    await MessageBox.Show(Window!, Se.Language.Video.AudioToText.NoVideoFile, Se.Language.Video.AudioToText.NoVideoFileFound);
                });
    ...
    private async Task ShowUnableToStartEngineErrorAsync()
    {
        ...
        if (engine.CanBeDownloaded() && !engine.IsEngineInstalled())
        {
            var answer = await MessageBox.Show(
                Window!,
                string.Format(Se.Language.Video.AudioToText.UnableToStartX, engine.Name),
                string.Format(Se.Language.Video.AudioToText.TheXProgramFileWasNotFoundX, engine.Name, Environment.NewLine) +
        ...
        {
            await MessageBox.Show(Window!, Se.Language.Video.AudioToText.UnknownError, string.Format(Se.Language.Video.AudioToText.UnableToStartX2, engine.Name));
        }
        else
        {
            await MessageBox.Show(Window!, Se.Language.General.Error, string.Format(Se.Language.Video.AudioToText.UnableToStartXX, engine.Name, error));
        }
    }
    ...
    private async Task ReDownloadWhisperEngine()
    {
        ...
        if (engine is ICrispAsrEngine && Configuration.IsRunningOnWindows)
        {
            var answer = await MessageBox.Show(
                Window,
                string.Format(Se.Language.Video.AudioToText.DownloadX, CrispAsrEngine.StaticName),
                string.Format(Se.Language.Video.AudioToText.XXRequiresDownloadingTheCrispASREngineX, Environment.NewLine, CrispAsrEngine.StaticName, Environment.NewLine, Environment.NewLine),
            ...
            if (crispVariant == "vulkan" && !VulkanHelper.IsInstalled())
            {
                var vulkanAnswer = await MessageBox.Show(
                    Window,
                    Se.Language.Video.AudioToText.VulkanSDKMayBeRequired,
                    string.Format(Se.Language.Video.AudioToText.TheVulkanVersionRequiresTheVulkanSDKTo, Environment.NewLine, Environment.NewLine, Environment.NewLine, Environment.NewLine, Environment.NewLine),
                    MessageBoxButtons.YesNoCancel,
                    MessageBoxIcon.Question);
    ...
    private async Task<bool?> PromptQwen3AsrGpuAsync(string engineName)
    {
        var answer = await MessageBox.Show(
            Window!,
            string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, engineName),
            string.Format(Se.Language.Video.AudioToText.XXRequiresDownloadingTheEngineXX, Environment.NewLine, engineName, Environment.NewLine, Environment.NewLine),
            MessageBoxButtons.Cancel,
            MessageBoxIcon.Question,
            "CPU",
            "GPU (Vulkan)");
        ...
        if (useVulkan && !VulkanHelper.IsInstalled())
        {
            var vulkanAnswer = await MessageBox.Show(
                Window!,
                Se.Language.Video.AudioToText.VulkanMayBeRequired,
    ...
    private async Task<string?> PromptCrispAsrLinuxVariantAsync(string engineName)
    {
        var answer = await MessageBox.Show(
            Window!,
            string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, engineName),
            string.Format(Se.Language.Video.AudioToText.XXRequiresDownloadingTheCrispASREngineX2, Environment.NewLine, engineName, Environment.NewLine, Environment.NewLine),
            MessageBoxButtons.Cancel,
            MessageBoxIcon.Question,
            "CPU",
            "CUDA");
    ...
    private async Task<string?> PromptCrispAsrCpuFlavorAsync()
    {
        var cpuAnswer = await MessageBox.Show(
            Window!,
            Se.Language.Video.AudioToText.CrispASRCPUBuild,
            string.Format(Se.Language.Video.AudioToText.XStandardIsRecommendedForMostMachinesX, Environment.NewLine, Environment.NewLine, Environment.NewLine),
            MessageBoxButtons.Cancel,
            MessageBoxIcon.Question,
            "Standard",
            "Legacy");
    ...
    private async Task Add()
    {
        ...
        {
            await MessageBox.Show(Window!,
                 Se.Language.Video.AudioToText.UnableToGetVideoInfo,
                 Se.Language.Video.AudioToText.FileSkippedAsVideoInfoWasUnavailable,
             MessageBoxButtons.OK,
    ...
    private async Task Transcribe()
    {
        ...
        if (engine is not IOnlineSttEngine)
        {
            ...
            if (!engine.IsEngineInstalled())
            {
                if (engine is MlxWhisperMac)
                {
                    ...
                    await MessageBox.Show(
                        Window!,
                        string.Format(Se.Language.Video.AudioToText.XNotFound, engine.Name),
                        Se.Language.Video.AudioToText.SubtitleEditCouldNotFindAPythonThat + Environment.NewLine +
                        Environment.NewLine +
                        Se.Language.Video.AudioToText.IfItIsNotInstalledYetInstallIt + Environment.NewLine +
                        "    pip3 install mlx-whisper" + Environment.NewLine +
                        Environment.NewLine +
                        Se.Language.Video.AudioToText.IfYouInstalledItWithPipxAVirtual +
                        Se.Language.Video.AudioToText.AutomaticallyOnlyWhenTheMlxWhisperCommandIs +
                ...
                if (engine is ICrispAsrEngine && Configuration.IsRunningOnWindows)
                {
                    var answer = await MessageBox.Show(
                        Window!,
                        string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, engine.Name),
                        string.Format(Se.Language.Video.AudioToText.XXRequiresDownloadingTheCrispASREngineX3, Environment.NewLine, engine.Name, Environment.NewLine, Environment.NewLine),
                        MessageBoxButtons.Cancel,
                        MessageBoxIcon.Question,
                        "CPU",
                        "Vulkan",
                ...
                else
                {
                    ...
                    else
                    {
                        var answer = await MessageBox.Show(
                            Window!,
                            string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, engine.Name),
                            string.Format(Se.Language.Video.AudioToText.DownloadAndUseX, engine.Name),
                            MessageBoxButtons.YesNoCancel,
                            MessageBoxIcon.Question);
            ...
            if (!engine.IsModelInstalled(model.Model))
            {
                var answer = await MessageBox.Show(
                    Window!,
                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, model),
                    string.Format(Se.Language.Video.AudioToText.DownloadAndUseX2, model.Model.Name),
                    MessageBoxButtons.YesNoCancel,
                    MessageBoxIcon.Question);
            ...
            if (engine is ChatLlmCppEngine chatLlm)
            {
                ...
                if (!engine.IsModelInstalled(modelAligner))
                {
                    var answer = await MessageBox.Show(
                                    Window!,
                                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, modelAligner),
                                    string.Format(Se.Language.Video.AudioToText.ChatLLMRequiresAForcedAlignerToCreate, modelAligner.Name),
                                    MessageBoxButtons.YesNoCancel,
                                    MessageBoxIcon.Question);
            ...
            if (engine is Qwen3AsrCppEngine qwen3Asr)
            {
                ...
                if (!engine.IsModelInstalled(modelAligner))
                {
                    var answer = await MessageBox.Show(
                                    Window!,
                                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, modelAligner),
                                    string.Format(Se.Language.Video.AudioToText.Qwen3ASRCPPRequiresAForcedAlignerTo, modelAligner.Name),
                                    MessageBoxButtons.YesNoCancel,
                                    MessageBoxIcon.Question);
                ...
                && (SelectedForcedAligner == null || SelectedForcedAligner.IsBuiltIn))
            {
                ...
                if (!engine.IsModelInstalled(modelAligner))
                {
                    var answer = await MessageBox.Show(
                                    Window!,
                                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, modelAligner),
                                    string.Format(Se.Language.Video.AudioToText.CrispASRQwen3RequiresAForcedAlignerTo, modelAligner.Name),
                                    MessageBoxButtons.YesNoCancel,
                                    MessageBoxIcon.Question);
                ...
                && (SelectedForcedAligner == null || SelectedForcedAligner.IsBuiltIn))
            {
                ...
                if (!engine.IsModelInstalled(modelAligner))
                {
                    var answer = await MessageBox.Show(
                                    Window!,
                                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, modelAligner),
                                    string.Format(Se.Language.Video.AudioToText.CrispASRMegaRequiresAForcedAlignerTo, modelAligner.Name),
                                    MessageBoxButtons.YesNoCancel,
                                    MessageBoxIcon.Question);
                ...
                && SelectedForcedAligner != null && !SelectedForcedAligner.IsBuiltIn)
            {
                ...
                if (string.IsNullOrEmpty(alignerPath) || !File.Exists(alignerPath))
                {
                    ...
                    var answer = await MessageBox.Show(
                                    Window!,
                                    string.Format(Se.Language.Video.AudioToText.DownloadXPrompt, SelectedForcedAligner.BaseDisplay),
                                    string.Format(Se.Language.Video.AudioToText.XIsSelectedButNotInstalledNDownloadAnd, SelectedForcedAligner.BaseDisplay, SelectedForcedAligner.FileName),
                                    MessageBoxButtons.YesNoCancel,
                                    MessageBoxIcon.Question);
            ...
            if (language.Code != "en" && IsModelEnglishOnly(model.Model))
            {
                var answer = await MessageBox.Show(
                    ...
                    Se.Language.Video.AudioToText.EnglishModelShouldOnlyBeUsedWithEnglish,
                    MessageBoxButtons.YesNoCancel,
                    MessageBoxIcon.Question);
        ...
        if (!IsBatchMode)
        {
            ...
            if (mediaInfo.Tracks.Count(p => p.TrackType == FfmpegTrackType.Audio) == 0)
            {
                var answer = await MessageBox.Show(
                    Window!,
                    Se.Language.Video.AudioToText.NoAudioTrackFound,
                    string.Format(Se.Language.Video.AudioToText.NoAudioTrackWasFoundInX, _videoFileName),
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
