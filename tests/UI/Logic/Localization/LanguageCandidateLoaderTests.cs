...
public sealed class LanguageCandidateLoaderTests
{
    ...
    [AvaloniaFact] public void WritableCandidateDoesNotMutateGlobals()=>AssertNoMutation(f=>{File.WriteAllText(Path.Combine(f,"Vietnamese.json"),Embedded());return new LanguageCandidateLoader(f).TryLoad("Vietnamese");});
    [AvaloniaFact] public void EmbeddedFallbackDoesNotMutateGlobals()=>AssertNoMutation(f=>new LanguageCandidateLoader(f).TryLoad("Vietnamese"));
    [AvaloniaFact] public void FailedNonEmbeddedCandidateDoesNotMutateGlobals()=>AssertNoMutation(f=>new LanguageCandidateLoader(f).TryLoad("Nope"));
    private static void AssertNoMutation(Func<string,LanguageLoadResult> load){WithFolder(f=>{var settings=Se.Settings;var language=Se.Language;var settingsJson=System.Text.Json.JsonSerializer.Serialize(settings);var languageJson=System.Text.Json.JsonSerializer.Serialize(language);load(f);Assert.Same(settings,Se.Settings);Assert.Same(language,Se.Language);Assert.Equal(settingsJson,System.Text.Json.JsonSerializer.Serialize(Se.Settings));Assert.Equal(languageJson,System.Text.Json.JsonSerializer.Serialize(Se.Language));});}
